<template>
  <div class="addImages" :class="bigBox">
    <form name="addImgs" id="form-addImg">
      <div class="showImages">
        <ul>
          ...
          <li class="upLoad" :class="contentBox" v-if="images.length<upNumber">
            <input name="selectImgs" type="file" id="fileImg" class="upLoad-button" v-if="!photoLoad" @change="up" @click="upLoad" accept="image/png,image/jpeg,image/jpg" />
            <input name="selectImgs" type="text" id="fileImg" class="upLoad-button" v-else @click="upLoad" accept="image/png,image/jpeg,image/jpg" />
            <img :src="url" />
          </li>
        </ul>
      </div>
    ...
    methods: {
      ...
      up: function() {
        ...
        for (var i = 0; i < file.length; i++) {
          ...
          //其他参数
          that.$ajax.post('https://files.uupt.com/u/fileupload.ashx?size=' + (that.size || "") + "&dir=" + (that.dir || ""), form, {
            method: 'post',
            headers: {
        ...
        document.getElementById("form-addImg").reset();
      },
