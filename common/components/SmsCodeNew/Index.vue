    ...
    methods: {
        sendSms: function() {
            ...
            validate.submit([
                ...
                that.disabled = true;
                Util.post({
                    url: that.commonUrl + "/Handler/Common.ashx?action=sendsms",
                    data: { mobile: that.mobile, validatetype: 1, bizcode: that.businessCode, captcha: that.captcha, },
                    success: function(result) {
                        if (result.IsError) {
                            ...
                            if (result.Msg == '图形验证码不对' || result.State == -999) {
                                ...
                                that.captchaUrl = that.commonUrl + "/Handler/Captcha.ashx?mobile=" + that.mobile + "&r=" + Math.random();
                                if (!that.captchashow) {
                                    that.captcha = '';
                                ...
                                that.captchaUrl = that.captchaUrl;
