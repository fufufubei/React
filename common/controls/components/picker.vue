<template>
  <transition name="uu-picker-fade">
    <uu-popup
        type="picker"
        :mask="true"
        :center="false"
        v-show="isVisible"
        @touchmove.prevent
        @mask-click="cancel" :isVisible="isVisible">
      <transition name="uu-picker-move">
        <div class="uu-picker-panel uu-safe-area-pb" v-show="isVisible" @click.stop>
          <div class="uu-picker-choose border-bottom-1px">
            <span data-action="cancel" @click="cancel">{{cancelTxt}}</span>
            <span data-action="confirm" @click="confirm">{{confirmTxt}}</span>
            <h1>{{title}}</h1>
          </div>
          <div class="uu-picker-content">
            <i class="border-bottom-1px"></i>
            <i class="border-top-1px"></i>
            <div class="uu-picker-wheel-wrapper" ref="wheelWrapper">
          ...
          <div class="uu-picker-footer"></div>
        </div>
      </transition>
    </uu-popup>
  </transition>
...
<script>
  ...
  const COMPONENT_NAME = 'uu-picker'
...
<style lang="stylus" rel="stylesheet/stylus">
  ...
  $picker-lr-padding = 16px*2
  .uu-picker-fade-enter, .uu-picker-fade-leave-active
    opacity: 0
  .uu-picker-fade-enter-active, .uu-picker-fade-leave-active
  ...
  .uu-picker-move-enter, .uu-picker-move-leave-active
    transform: translate3d(0, 273px*2, 0)
  .uu-picker-move-enter-active, .uu-picker-move-leave-active
    transition: all .3s ease-in-out
  .uu-picker-choose
    position: relative
    height: 60px*2
    > span
      position: absolute
      top: 6px*2
      padding: 16px*2 $picker-lr-padding
      font-size: $fontsize-medium
    > [data-action="confirm"]
      right: 0
